...
def get_prompt_section(title: str) -> list[str]:
    """Extracts a section from the queen_system.md file by its ## title."""
    content = _load_system_prompt_file()
    marker = f"## {title}"
...
def build_bootstrap_context_prompt(store: "Store", chat_id: int) -> BootstrapContext:
...
async def build_queen_prompt(
    ...
    system_prompt = _load_system_prompt_file()
    persona_prompt_lines = build_persona_prompt()
