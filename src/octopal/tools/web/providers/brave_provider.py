...
def is_configured(api_key: str | None = None) -> bool:
    return bool((api_key if api_key is not None else os.getenv("BRAVE_API_KEY") or "").strip())
...
def _prepare_request(
    ...
    resolved_api_key = (
        api_key if api_key is not None else os.getenv("BRAVE_API_KEY") or ""
    ).strip()
    ...
    return query, params, resolved_api_key
