    ...
    *,
    ...
    try:
        ...
        if plan:
            ...
            if plan["mode"] == "reply":
                plan_response = str(plan.get("response", ""))
                if await _needs_action_or_blocked_retry(
                    provider=provider,
                    messages=messages,
                    candidate=plan_response,
                ):
                    logger.warning(
                        "Planner reply requires runtime action; continuing through execution route",
                        preview=plan_response[:200],
                    )
                    plan = None
                else:
