...
def start(
    ...
    if running_pids:
        console.print("[bold yellow]Octopal is already running.[/bold yellow]")
...
def _start_background() -> None:
    ...
    console.print("[bold cyan]Starting Octopal in background...[/bold cyan]")

    # Use the current python executable and run the module with --foreground
    args = [sys.executable, "-m", "octopal.cli", "start", "--foreground"]
    ...
    with (
        ...
        try:
            ...
            time.sleep(2)
            console.print("[bold green][V] Octopal started in background.[/bold green]")
            console.print("Use [magenta]octopal status[/magenta] to check status.")
            console.print("Use [magenta]octopal logs -f[/magenta] to view logs.")
        except Exception as e:
...
def stop() -> None:
    ...
    if not targets:
        console.print("[yellow]Octopal is not running.[/yellow]")
        return

    console.print(
        f"[bold yellow]Stopping Octopal ({len(targets)} process(es)): "
    ...
    try:
        ...
        if failures:
            console.print("[bold red]Failed to stop all Octopal processes:[/bold red]")
            for failed_pid, reason in failures:
                console.print(f" - PID {failed_pid}: {reason}")
            raise typer.Exit(code=1)

        console.print("[bold green][V] Octopal stopped.[/bold green]")
    except typer.Exit:
        raise
    except Exception as e:
        console.print(f"[bold red]Failed to stop Octopal: {e}[/bold red]")
        raise typer.Exit(code=1) from e
...
def config_show(reveal_secrets: bool = typer.Option(False, "--reveal-secrets", help="Show API keys and tokens")) -> None:
    ...
    groups: list[tuple[str, set[str]]] = [
        (
            "User Channel",
            {
                "OCTOPAL_USER_CHANNEL",
                "TELEGRAM_BOT_TOKEN",
                "ALLOWED_TELEGRAM_CHAT_IDS",
                "OCTOPAL_TELEGRAM_PARSE_MODE",
                "OCTOPAL_WHATSAPP_MODE",
                "ALLOWED_WHATSAPP_NUMBERS",
                "OCTOPAL_WHATSAPP_AUTH_DIR",
                "OCTOPAL_WHATSAPP_BRIDGE_HOST",
                "OCTOPAL_WHATSAPP_BRIDGE_PORT",
                "OCTOPAL_WHATSAPP_CALLBACK_TOKEN",
                "OCTOPAL_WHATSAPP_NODE_COMMAND",
            },
        ),
        (
            "Provider",
        ...
        (
            "Runtime",
            {
                ...
                "OCTOPAL_LOG_LEVEL",
                "OCTOPAL_HEARTBEAT_INTERVAL_SECONDS",
                "OCTOPAL_GATEWAY_HOST",
                "OCTOPAL_GATEWAY_PORT",
                "OCTOPAL_DASHBOARD_TOKEN",
                "OCTOPAL_TAILSCALE_IPS",
                "OCTOPAL_TAILSCALE_AUTO_SERVE",
            },
        ),
        (
            "Workers",
            {
                "OCTOPAL_WORKER_LAUNCHER",
                "OCTOPAL_WORKER_DOCKER_IMAGE",
                "OCTOPAL_WORKER_DOCKER_WORKSPACE",
                "OCTOPAL_WORKER_MAX_SPAWN_DEPTH",
                "OCTOPAL_WORKER_MAX_CHILDREN_TOTAL",
                "OCTOPAL_WORKER_MAX_CHILDREN_CONCURRENT",
            },
        ),
    ]
    ...
    if resolved_profile.requires_api_key and not (resolved_profile.api_key or "").strip():
        checks.append(
            f"[bright_red]Set OCTOPAL_LITELLM_API_KEY (or legacy key) for {resolved_profile.label}[/bright_red]"
        )
    if not (resolved_profile.model or "").strip():
        checks.append("[bright_red]OCTOPAL_LITELLM_MODEL is not set[/bright_red]")
    if not checks:
...
@app.command()
def build_worker_image(tag: str = "octopal-worker:latest") -> None:
